...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	q := Queries{db: db}
	var err error
	if q.countBooksStmt, err = db.PrepareContext(ctx, countBooks); err != nil {
		return nil, fmt.Errorf("error preparing query CountBooks: %w", err)
	}
	...
	if q.searchBooksByAuthorStmt, err = db.PrepareContext(ctx, searchBooksByAuthor); err != nil {
		return nil, fmt.Errorf("error preparing query SearchBooksByAuthor: %w", err)
	}
	if q.searchBooksByTitleStmt, err = db.PrepareContext(ctx, searchBooksByTitle); err != nil {
		return nil, fmt.Errorf("error preparing query SearchBooksByTitle: %w", err)
	}
...
func (q *Queries) Close() error {
	var err error
	if q.countBooksStmt != nil {
		if cerr := q.countBooksStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing countBooksStmt: %w", cerr)
		}
	}
	...
	if q.searchBooksByAuthorStmt != nil {
		if cerr := q.searchBooksByAuthorStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing searchBooksByAuthorStmt: %w", cerr)
		}
	}
	if q.searchBooksByTitleStmt != nil {
		if cerr := q.searchBooksByTitleStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing searchBooksByTitleStmt: %w", cerr)
		}
	}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		db:                      tx,
		tx:                      tx,
		countBooksStmt:          q.countBooksStmt,
		createBookStmt:          q.createBookStmt,
		deleteBookStmt:          q.deleteBookStmt,
		getBookStmt:             q.getBookStmt,
		listBooksStmt:           q.listBooksStmt,
		searchBooksByAuthorStmt: q.searchBooksByAuthorStmt,
		searchBooksByTitleStmt:  q.searchBooksByTitleStmt,
